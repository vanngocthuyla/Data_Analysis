...
#options(echo=FALSE)
#args <- commandArgs(trailingOnly = TRUE) #use the file PCA_R_call
...
for (i in 1:(todraw-1)) {
    for (j in (i+1):todraw) {
        plot = ggplot (data.frame (pca$x), aes_string(x=colnames (pca$x) [i], y=colnames(pca$x)[j], colour = "Group")) + geom_point() + labs(x=newnames_prt[i], y=newnames_prt[j]) + theme(legend.position="bottom")
        print (plot)
    }
}
